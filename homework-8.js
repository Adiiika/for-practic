...
const productNamesList = productCards.reduce((acc, card) => {
    acc.push(card.name)
    return acc
}, [])

console.log(productNamesList.join(";"));

// Используя метод .reduce(), получить массив объектов, где ключем является название продукта, а значением - его описание
const productKeysAndValues = productCards.reduce((acc, card) => {
    acc.push({ [card.name]: card.description })
    return acc
},[])
console.log(productKeysAndValues)
...
productCards.forEach(product => {
    ...
    productCardsClone.querySelector('.product-compound').innerHTML = product.compound.map(compound => `<li>${compound}</li>`).join('')
    productCardsClone.querySelector('.product-list-item').textContent = product.listItem 
